...
fn golden_chain_advance_vector_0() {
    ...
    let expected_msg =
        hex_to_32("a4e53f48d6a8f6f992802dc0dab2ac6aaac4fa3baf75c8c02f4f60a273fcdfb1");
    let expected_next =
        hex_to_32("aa659d2f3fc3391c814e634b230e1a3160e0be9df47d78912284064f2746f033");
    let result = chain_advance(&chain_key);
    assert_eq!(result.message_key, expected_msg, "chain-step-0 message_key");
    assert_eq!(
        result.next_chain_key, expected_next,
        "chain-step-0 next_chain_key"
    );
}
...
fn golden_chain_advance_vector_1() {
    ...
    let expected_msg =
        hex_to_32("e0a8b8b0ea44123935b1f88a424c433acdb381e7856ddc436251332aa56d9fec");
    let expected_next =
        hex_to_32("80c50c2d3d490ec0e26163ef84216a7da12c0888ae7062ee052653e567cfe516");
    let result = chain_advance(&chain_key);
    assert_eq!(result.message_key, expected_msg, "chain-step-1 message_key");
    assert_eq!(
        result.next_chain_key, expected_next,
        "chain-step-1 next_chain_key"
    );
}
